...
eval "$(ssh-agent -s)" > "$LOG_DIR/network-skripts-log.txt"
...
while true; do
   ...
   git fetch origin "$BRANCH" >> "$LOG_DIR/network-skripts-log.txt" 2>&1
    ...
    if [ "$CURRENT_COMMIT" != "$LAST_COMMIT" ]; then
        echo "Changes detected on branch '$BRANCH'! Performing action..." >> "$LOG_DIR/network-skripts-log.txt" 2>&1
        ...
        git pull origin "$BRANCH" >> "$LOG_DIR/network-skripts-log.txt" 2>&1
        
    else
        echo "No changes detected on branch '$BRANCH'." >> "$LOG_DIR/network-skripts-log.txt" 2>&1
    fi
